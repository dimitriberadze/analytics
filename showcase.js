...
const COLORS = ['#ff600a', '#7938ea', '#06a74c', '#ffbd23', '#e22820', '#2c4160', '#ccff33', '#43291f', '#f72585', '#e0e8fc'];

const light = {
	chart: { backgroundColor: '#fcfcfc' },
	title: { style: { fontFamily: 'BOG', fontWeight: '700', fontSize: '16px', lineHeight: '20px', color: 'rgba(1, 1, 1, 0.68)' } },
	subtitle: { style: { fontFamily: 'BOG', fontWeight: '500', fontSize: '12px', lineHeight: '20px', color: 'rgba(0, 0, 0, 0.3)' } },
	xAxis: { labels: { style: { fontFamily: 'BOG', fontWeight: '500', fontSize: '10px', lineHeight: '16px', color: 'rgba(0, 0, 0, 0.9)' } } },
	yAxis: {
		gridLineColor: '#cccccc',
		labels: { style: { fontFamily: 'BOG', fontWeight: '500', fontSize: '10px', lineHeight: '16px', color: 'rgba(0, 0, 0, 0.9)' } },
		title: { style: { fontFamily: 'BOG', fontWeight: '500', fontSize: '10px', lineHeight: '16px', textOutline: 0, color: '#6c7075' } },
	},
	legend: {
		itemStyle: { fontFamily: 'BOG', fontWeight: '500', fontSize: '12px', lineHeight: '20px', color: 'rgba(0, 0, 0, 0.9)', cursor: 'default' },
...
const STRINGS = {
	ka: {
		...
		uctrTip: 'მომხმარებლების %, რომლებმაც გახსნეს შეთავაზების დეტალები. არ ითვლება ერთი მომხმარებლის მიერ რამდენჯერმე გახსნა.',
		freqText: 'ნახვების სიხშირე',
		freqTip: 'საშუალოდ რამდენჯერ ნახა თითოეულმა მომხმარებელმა თქვენი შეთავაზება. გეხმარებათ გაიგოთ, რამდენად ხშირად/იშვიათად უჩვენებთ შეთავაზებას',
		...
		ipSubOn: 'თქვენს შეთავაზებას დაწყებიდან {var1} დღეში {var2} ნახვა აქვს. მსგავსი შეთავაზებები ამ პერიოდში საშუალოდ {var3} ნახვას აგროვებს. შეთავაზება მეტ მომხმარებელს მიაღწევს, თუმცა მათი ინტერესი ჯერ დაბალია.',
		ipTitleBelow: 'შედეგები მოლოდინს ჩამორჩება',
		ipSubBelow: 'თქვენს შეთავაზებას დაწყებიდან {var1} დღეში {var2} ნახვა აქვს, მოსალოდნელი {var3}-ის ნაცვლად. განიხილეთ სამიზნეობის ან ბიუჯეტის ცვლილება.',
		ipTitleExceeds: 'შესანიშნავი შედეგებია',
		ipSubExceeds: 'თქვენს შეთავაზებას დაწყებიდან {var1} დღეში {var2} ნახვა აქვს, მოსალოდნელი {var3}-ის ზემოთ. შეთავაზება ძალიან კარგად მუშაობს.',
	...
	en: {
		...
		freqTip: 'On average, how many times each user saw your offer. Helps you understand how often/rarely the offer is shown.',

		learningTitle: 'Offer assessment',
		learningSubtitle: 'Within 3 days of launch you can see how well your offer performs against similar offers from other companies.',
		...
		ipSubOn: 'Your offer has {var2} views in the first {var1} days. Similar offers gather {var3} views on average in this period. The offer will reach more users, but their interest is still low.',
		ipTitleBelow: 'Below expectations',
		ipSubBelow: 'Your offer has {var2} views in the first {var1} days, instead of the expected {var3}. Consider revisiting targeting or budget.',
		ipTitleExceeds: 'Excellent results',
		ipSubExceeds: 'Your offer has {var2} views in the first {var1} days, above the expected {var3}. The offer is performing very well.',
...
function bucketsForRange(start, end, periodType) {
	...
	if (periodType === 'WEEK') {
		...
		for (let i = 0; i < days.length; i += 7) {
			...
			out.push({ date: [chunk[0].format('YYYY/MM/DD'), chunk[chunk.length - 1].format('YYYY/MM/DD')], days: chunk });
		}
		return out;
	}
...
function dailyLearning(idx, factor) {
	const base = 700 + Math.sin(idx / 2.5) * 250 + Math.cos(idx / 7) * 150;
	return { predicted: Math.max(60, Math.round(base)), views: Math.max(0, Math.round(base * factor + Math.sin(idx) * 80)) };
}

function dailyCtr(idx) {
	const views = Math.max(0, Math.round(1800 + Math.sin(idx / 2) * 500 + Math.cos(idx / 9) * 300));
	const clicks = Math.max(0, Math.round(views * (0.045 + Math.sin(idx) / 70)));
	return { views, clicks };
}
...
function buildCtrTime({ startDate, endDate, periodType }) {
	...
	return { data, periodType: pt, totalViews, totalClicks, totalCtrPercentage: totalViews ? Math.round((totalClicks / totalViews) * 1000) / 10 : 0 };
}
...
function route(url, params) {
	...
	if (url.includes('impressions/summary')) {
		return empty ? { noData: true } : { result: { returnOnAdSpend: 0, conversionRate: 7, ctr: 5, uniqueCtr: 5.44, frequency: 1.14 } };
	}
	if (url.includes('impressions/devices/chart')) {
		return { result: empty ? { noData: true } : { impressions: [{ device: 'android', impressions: 62 }, { device: 'ios', impressions: 38 }] } };
	}
	...
	if (url.includes('offer/realization')) {
		...
		return { result: [{ channelId: 13, amount: Math.round(240 * s) }, { channelId: -99, amount: Math.round(180 * s) }, { channelId: 1, amount: Math.round(40 * s) }] };
	}
...
class ShowcaseLearningChart extends LitElement {
	...
	get infoPanel() {
		...
		return html`<aside class="info-panel info-panel--${perf.toLowerCase()}${this.isDetailsPage ? ' info-panel--details' : ''}">
		...
		try {
			const data = await mockApi.execute({
				...
				params: {
					...
					startDate: this.filterData?.startDate ? dayjs(this.filterData.startDate).format('YYYY-MM-DD') : undefined,
...
class ShowcaseCtrTimeChart extends LitElement {
		...
		return html`
			${this.loading
				...
				: html`${this.chartHeaderPart}
						<div class="chart-layout"><div class="chart-card"><div id="chart"></div></div></div>`}
		`;
	}
	...
	_modifyChartData(input) {
		const t = this.t;
		return [
			{ name: t('ctrLegendViews'), type: 'column', color: CTR_SERIES_COLORS.views, data: input.data.map((i) => i.views), yAxis: 0 },
			{ name: t('ctrClick'), type: 'column', color: CTR_SERIES_COLORS.clicks, data: input.data.map((i) => i.clicks), yAxis: 0 },
			{
				name: t('ctrCtr'),
				type: 'spline',
	...
	async _fetchChartData() {
		...
		try {
			...
			this.aggregateData = { views: data.json.totalViews, clicks: data.json.totalClicks, ctr: data.json.totalCtrPercentage };
			this.view = VIEWS.SUCCESS;
			this._initChart();
		} catch (e) {
			console.error(e);
		...
		requestAnimationFrame(() => {
			...
			this.chart = Highcharts.chart(container, {
				...
				yAxis: [
					...
					{ min: 0, opposite: true, labels: { format: '{value}%' }, title: { text: t('ctrCtr') }, gridLineWidth: 0, lineWidth: 0 },
				],
				tooltip: {
					shared: true,
					useHTML: true,
...
class ShowcaseDeviceChart extends LitElement {
		...
		return html`
			${this.loading
				...
				: html`<div class="chart-card">
						...
						<div class="legend">
							${this.chartData?.map(
								(d) => html`<div class="legend-item">
									<span class="legend-point" style="background-color: ${d.y === 0 ? getCssVariableColor('--text-disabled') : d.color};"></span>
	...
	async _fetchChartData() {
		this.loading = true;
		try {
			const res = await mockApi.execute({ url: 'ads-manager/impressions/devices/chart', params: { empty: this.emptyState } });
			if (this.chart && Object.keys(this.chart).length) this.chart.destroy();
...
class ShowcaseRealizationChart extends LitElement {
	...
	async _fetch() {
		this.loading = true;
		try {
			const data = await mockApi.execute({ url: 'ads-manager/offer/realization', params: { empty: this.emptyState, periodId: this.periodId } });
			if (this.chart && Object.keys(this.chart).length) this.chart.destroy();
						...
						render() {
							...
							if (!label) {
								label = chart.options.chart.custom.label = chart.renderer
									.label(`<div class="custom-label"><span class="custom-text">${title}</span><span class="custom-sum">${sum}</span></div>`, 0, 0, 'rect', null, null, true)
									.add();
							}
							label.attr({ x: (chart.plotWidth - label.width) / 2, y: (chart.plotHeight - label.height) / 2 });
						},
					},
				},
				title: { text: null },
				subtitle: { text: null },
				tooltip: { useHTML: true, pointFormat: '<b>{point.y}</b>', backgroundColor: '#000000C7', borderRadius: 4, style: { color: '#fff' } },
				legend: { enabled: false },
				plotOptions: { series: { borderRadius: 10, borderWidth: 5, borderColor: '#fcfcfc', dataLabels: { enabled: false } } },
				series: [{ colorByPoint: true, innerSize: '65%', size: '100%', data: this.chartData }],
				credits: { enabled: false },
				colors: COLORS,
			});
...
class ShowcaseOfferUsageChart extends LitElement {
	...
	async _fetch() {
		this.loading = true;
		try {
			const res = await mockApi.execute({ url: 'ads-manager/impressions/engagement/chart', params: { empty: this.emptyState, periodId: this.periodId } });
			if (this.chart && Object.keys(this.chart).length) this.chart.destroy();
			...
			this.chartData = Object.entries(result)
				...
				.map(([key, value]) => ({ name: this.t(USAGE_MAP[key].key), color: USAGE_MAP[key].color, data: [value] }));
			this.view = VIEWS.SUCCESS;
			this._initChart();
		} catch (e) {
			console.error(e);
	...
	_initChart() {
		...
		requestAnimationFrame(() => {
			...
			this.chart = Highcharts.chart(container, {
				...
				tooltip: { useHTML: true, headerFormat: '', pointFormat: '<b>{point.y}</b>', backgroundColor: '#000000C7', borderRadius: 4, style: { color: '#fff' } },
				plotOptions: {
		...
		return html`
			<section class="charts">
				...
				<div class="charts-content">
					<showcase-chart-wrapper class="chart" show-header header-title="${t('realizationTitle')}" header-content="${t('realizationDesc')}">
					...
					<showcase-chart-wrapper class="chart" show-header header-title="${t('usageTitle')}" header-content="${t('usageDesc')}">
			...
			<section class="transactions">
				<div class="tx-header">
					...
					<bd-standard-button size="small" type="text-primary" prefix-icon="bd:download" ?disabled="${!this._rows.length}"
					...
					<div class="flt-actions">
						${this._hasFilters
							? html`<bd-standard-button type="text-low" size="small" @click="${this._clearFilters}"
							  ...
							  >`
							: nothing}
						<bd-standard-button size="small" type="primary" @click="${this._applyFilters}">${t('fltApply')}</bd-standard-button>
					</div>
				</div>
				...
				<bd-table mode="basic" class="tx-table">
					<bd-table-row class="row" header>
						...
						<bd-table-cell class="amount"><bd-font type="overline2">${t('colAmount')}</bd-font></bd-table-cell>
					</bd-table-row>
					${this._rows.map(
						(r) => html`<bd-table-row class="row">
							<bd-table-cell><bd-font type="button-text">${r.rrn}</bd-font></bd-table-cell>
							<bd-table-cell><bd-font type="button-text">${formatDate(r.authDate, 'D MMM, YYYY', this.lang)}</bd-font></bd-table-cell>
							<bd-table-cell><bd-font type="button-text">${t(r.channel === 'online' ? 'chOnline' : 'chPos')}</bd-font></bd-table-cell>
							<bd-table-cell><bd-font type="button-text">${r.cardGroup}</bd-font></bd-table-cell>
							<bd-table-cell class="amount"><bd-font type="number">${r.fee} ₾</bd-font></bd-table-cell>
							<bd-table-cell class="amount"><bd-font type="number">${r.amount} ₾</bd-font></bd-table-cell>
						</bd-table-row>`,
					)}
				</bd-table>
		...
		return html`
				...
				<section>
					...
					<div class="rows">
						<div class="row row-inline">
							...
							<bd-system-badge type="success" text="${t('odPackageValue')}" small ?hidePrefixIcon="${true}"></bd-system-badge>
						</div>
						...
						<div class="row">
							...
							<bd-standard-button type="text-primary" size="small" prefix-icon="bd:download">${t('odConditionsLink')}</bd-standard-button>
						</div>
						<div class="row">
			...
			<div class="col">
				...
				<section>
					...
					<div class="rows">
						${this._row(t('odUrl'), t('odUrlValue'))} ${this._row(t('odContact'), '-')} ${this._row(t('odMainMsg'), '-')}
						<div class="row">
...
class ShowcaseAnalyticsSection extends LitElement {
	...
	constructor() {
		...
		this.emptyState = false;
		this.sumInfoData = {};
		this.campaignFilterData = { startDate: dayjs(OFFER.start, 'YYYY/MM/DD').toDate(), endDate: dayjs(OFFER.end, 'YYYY/MM/DD').toDate() };
	}
	...
	render() {
		...
		return html`
			...
			<div class="offer-analytics-charts-wrapper">
				...
				<showcase-chart-wrapper show-header header-title="${t('deviceTitle')}" header-content="${t('deviceSubtitle')}">
					<showcase-device-chart slot="chart" .lang="${this.lang}" .emptyState="${this.emptyState}"></showcase-device-chart>
				</showcase-chart-wrapper>
			</div>
		`;
	}
			...
			${cards.map((c) => {
				...
				return html`
					<bd-numbers-card
						...
						with-tooltip
					>
						<bd-font type="number" slot="content"><span>${value == null ? '—' : c.pct ? `${value}%` : value}</span></bd-font>
					</bd-numbers-card>
		...
		return html`
			<div class="card">
				<div class="header">
					<div class="header-text">
						<div class="texts">
							<bd-icon-button class="back-icon" type="secondary-grey" icon="bd:chevron_left" size="small"></bd-icon-button>
					...
					<div class="header-actions">
						<bd-assist-chips class="chips" icon="bd:copy_outline" text="${t('actionDuplicate')}"></bd-assist-chips>
				...
				<div class="offer-summary">
					...
					<div class="offer-rows">
						<bd-list-item-v2 static type="rounded" .subtitle="${t('infoOffer')}" .title="${t('offerText')}"></bd-list-item-v2>
						<bd-list-item-v2 static type="rounded" .subtitle="${t('infoPeriod')}" .title="${t('offerPeriod')}"></bd-list-item-v2>
					</div>
				</div>
				...
				<div class="offer-numbers">
					${[t('numRevenue'), t('numBenefit'), t('numVat'), t('numCost')].map(
						(label) => html`<bd-numbers-card class="numbers-card" type="gray" size="large" text-only text="${label}">
			...
			<div class="card tabs-card">
				<div class="controller-items">
					...
					>
						<bd-tab slot="tab" name="analytics" align="center" icon="bd:chart_outline" @click="${() => (this.selectedTab = 'analytics')}"
...
class DetailsBase extends LitElement {
	...
	_header() {
		return html`<header class="info-header">
			<div class="breadcrumbs-back">
				<bd-icon-button class="back-icon" type="secondary-grey" icon="bd:chevron_left" size="small" @click="${this._back}"></bd-icon-button>
	...
	_renderPage(chart, tableTitle) {
		const t = this.t;
		return html`
			<div class="breadcrumbs-wrapper"><bd-breadcrumbs-v2 .options="${this._breadcrumbsOptions}"></bd-breadcrumbs-v2></div>
				...
				<main class="info-content">
					...
					<div class="table-wrapper">
						<div class="table-header">
							...
							<bd-standard-button size="small" type="text-primary" prefix-icon="bd:download" ?disabled="${this.tableData.noData}"
...
class ShowcaseLearningDetails extends DetailsBase {
	...
	_compareValues(data) {
		...
		return {
			...
			data: data.data.map((item) => ({
				...item,
				status: item.views > item.predictedViews ? 'above' : item.views < item.predictedViews ? 'below' : 'equal',
			})),
		};
	}
	...
	_tablePart() {
		...
		return html`<bd-table class="details-table" mode="basic" style="--cell-template-columns: repeat(3, 1fr);">
			<bd-table-row class="row" header>
				${titles.map(
					(item, i) => html`<bd-table-cell class="secondary-title cell ${i === titles.length - 1 ? 'last-cell' : ''}">
						<bd-font type="overline2"><span>${item.label}</span></bd-font>
					</bd-table-cell>`,
				)}
			</bd-table-row>
			${this.tableData.data.map(
				(item) => html`<bd-table-row class="row">
					<bd-table-cell class="cell">
						<bd-font type="button-text"><span>${formatDate(item.date[0], 'DD MMM/YYYY', this.lang)}</span></bd-font>
					</bd-table-cell>
					<bd-table-cell class="cell"><bd-font type="button-text"><span>${item.predictedViews}</span></bd-font></bd-table-cell>
					<bd-table-cell class="cell last-cell ${item.status}"><bd-font type="button-text"><span>${item.views}</span></bd-font></bd-table-cell>
				</bd-table-row>`,
			)}
		</bd-table>`;
	}
		...
		return html`<bd-table class="details-table" mode="basic" style="--cell-template-columns: repeat(4, 1fr);">
			...
			${this.tableData.data.map(
				(item) => html`<bd-table-row class="row">
					...
					<bd-table-cell class="cell"><bd-font type="button-text"><span>${item.views}</span></bd-font></bd-table-cell>
					<bd-table-cell class="cell"><bd-font type="button-text"><span>${item.clicks}</span></bd-font></bd-table-cell>
					<bd-table-cell class="cell last-cell"><bd-font type="button-text"><span>${item.ctrPercentage}%</span></bd-font></bd-table-cell>
				</bd-table-row>`,
			)}
		</bd-table>`;
	}
...
class AnalyticsShowcaseRoot extends LitElement {
	...
	_toolbar() {
		...
		const chip = (label, active, onClick) =>
			html`<bd-filter-chips text="${label}" ?selected="${active}" @on-chips-click="${onClick}"></bd-filter-chips>`;
		return html`<div class="toolbar">
			<div class="toolbar-group">
				<bd-font class="toolbar-label" type="small-text">ენა</bd-font>
				${chip('ქართული', this.lang === 'ka', () => (this.lang = 'ka'))} ${chip('English', this.lang === 'en', () => (this.lang = 'en'))}
			</div>
			<div class="toolbar-group">
				<bd-font class="toolbar-label" type="small-text">${t('learningTitle')}</bd-font>
				${chip(t('ipTitleBelow'), this.scenario === 'BELOW_EXPECTATIONS', () => (this.scenario = 'BELOW_EXPECTATIONS'))}
				${chip(t('ipTitleOn'), this.scenario === 'ON_TRACK', () => (this.scenario = 'ON_TRACK'))}
				${chip(t('ipTitleExceeds'), this.scenario === 'EXCEEDS_EXPECTATIONS', () => (this.scenario = 'EXCEEDS_EXPECTATIONS'))}
			</div>
			<div class="toolbar-group">
	...
	_view() {
		...
		if (this.route === 'ctr-time') {
			return html`<showcase-ctr-details .lang="${this.lang}" .emptyState="${this.emptyState}"></showcase-ctr-details>`;
		}
